window.AsUI={
 ...
 renderMenu(){const m=document.getElementById('menu');if(!m)return;m.innerHTML='<div class="brand">Asgard</div>'+this.menu.map(x=>'<div tabindex="0" class="nav-item focusable '+(x===this.state.screen?'active':'')+'" onclick="AsUI.nav(\''+x+'\')">'+this.escape(x)+'</div>').join('')},
