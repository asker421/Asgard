(function(){
  ...
  window.AsJacRedAdapter={
    ...
    endpoints(baseUrl){return [baseUrl,joinUrl(baseUrl,'api'),joinUrl(baseUrl,'torznab'),joinUrl(baseUrl,'api/torznab')]},
    async fetchText(url){const c=new AbortController();const timer=setTimeout(()=>c.abort(),9000);try{const res=await fetch(url,{signal:c.signal,headers:{Accept:'application/xml,text/xml,application/rss+xml,application/json,text/plain,*/*'}});const text=await res.text();return {ok:res.ok,status:res.status,contentType:res.headers.get('content-type')||'',text,url}}finally{clearTimeout(timer)}},
