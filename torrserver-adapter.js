(function(){
  ...
  window.AsTorrServerAdapter={
    ...
    base(){return String(AsStore.parserSettings().torrServerUrl||window.ASGARD_DEFAULT_TORR_SERVER||'').replace(/\/+$/,'')},
    ...
    async request(path,body){
      ...
      const poster=window.AsgardBridge&&AsgardBridge.nativePostJson;
      if(body&&poster){
        const raw=AsgardBridge.nativePostJson(url,JSON.stringify(body));
        const parsed=JSON.parse(raw||'{}');
        let data=parsed.body||'';try{data=JSON.parse(data)}catch(e){}
        return {ok:!!parsed.ok,status:parsed.status||0,data:data,text:parsed.body||'',url:parsed.url||url,contentType:parsed.contentType||'',error:parsed.error||'',native:true};
      }
      if(!body&&fetcher){
    ...
    async preparePlayableFromResult(result){
      ...
      try{
        ...
        const magnet=result&&result.magnetUrl||(String(result&&result.url||result&&result.target||'').startsWith('magnet:?')?(result.url||result.target):'');
        const torrentUrl=result&&result.torrentUrl||(!magnet?(result&&result.url||result&&result.target):'');
